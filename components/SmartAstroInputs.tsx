              ...
              {slots.map(time => {
                  ...
                  return (
                      ...
                      >
                          ...
                          <span className="text-xs text-blue-300">{lagna} Ascendant</span>
                      </div>
                  )
              })}
          </div>
...
export const SmartCitySearch: React.FC<SmartCitySearchProps> = ({ value, onChange }) => {
  ...
  const handleSelect = (city: City) => {
    ...
    const recentsStr = safeStorageInstance.getItem('glyph_recent_cities') || '[]';
    const recents = JSON.parse(recentsStr);
    const newRecents = [city, ...recents.filter((c: City) => c.name !== city.name)].slice(0, 5);
    safeStorageInstance.setItem('glyph_recent_cities', JSON.stringify(newRecents));
  };

  const showRecents = () => {
      const recentsStr = safeStorageInstance.getItem('glyph_recent_cities') || '[]';
      setSuggestions(JSON.parse(recentsStr));
      setIsOpen(true);
  };
