// src/components/reports/DesktopReport.tsx
// Desktop-specific report with A4 format and zoom controls
...
const DesktopReport: React.FC<DesktopReportProps> = ({
    reading,
    title,
    subtitle,
    chartData,
    ...
    const containerId = useMemo(() => {
        return `${category}-desktop-report`;
    }, [category]);
    ...
    // --- DESKTOP PAGINATION (A4 format) ---
    useEffect(() => {
        if (!segments.length) return;
        const chunks: string[][] = [[]];
        let currentChunk = 0;
        let charCount = 0;
        const MAX_CHARS_PER_PAGE = 2200; // Desktop optimized
    ...
    // Custom markdown components (Desktop optimized)
    const markdownComponents = useMemo(() => ({
        ...
        h1: ({ node, children, ...props }: any) => (
            <h1 className="text-4xl font-cinzel font-black uppercase tracking-tight mt-8 mb-4"
                style={{ color: template?.content_area_config.textColor || '#4a0404' }} {...props}>
                {children}
            </h1>
        ),
        h2: ({ node, children, ...props }: any) => (
            <h2 className="text-3xl font-cinzel font-black uppercase tracking-tight mt-6 mb-3"
                style={{ color: template?.content_area_config.textColor || '#4a0404' }} {...props}>
                {children}
            </h2>
        ),
        h3: ({ node, children, ...props }: any) => (
            <h3 className="text-2xl font-cinzel font-bold uppercase tracking-tight mt-6 mb-3"
                style={{ color: template?.content_area_config.textColor || '#4a0404' }} {...props}>
                {children}
            </h3>
        ),
    ...
    const renderSegment = (line: string, index: number) => {
        ...
        if (isHeader) {
            return (
                <div key={index} className="w-full mt-12 mb-6">
                    <h4 className="text-4xl font-cinzel font-black text-center uppercase tracking-tight"
        ...
        return (
            <div key={index} className="relative flex items-start mb-4 p-4 rounded-xl">
                ...
                >
                    <div style={contentStyles} className="relative z-10 flex flex-col items-center">
                        ...
                        <div className="text-center px-10">
                            <h2 className="text-5xl font-cinzel font-black uppercase tracking-tight leading-tight"
