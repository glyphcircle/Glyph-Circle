...
const ConsultationBooking: React.FC = () => {
  ...
  console.log('🎯 [ConsultationBooking] Component mounted');
  console.log('📍 [ConsultationBooking] Current location:', location.pathname);
  ...
  const handleConsultationClick = (option: ConsultationOption) => {
    console.log('🔥 [ConsultationBooking] Button clicked!', {
      title: option.title,
    ...
    try {
      ...
      console.log('📦 [ConsultationBooking] Navigation state:', {
        returnPath,
        scrollPosition,
      ...
      navigate('/consultation-booking', {
        state: {
          ...
          serviceType: location.state?.serviceType || 
            (location.pathname.includes('astrology') ? 'astrology' : 'numerology'),
        },
      });

      console.log('✅ [ConsultationBooking] Navigation initiated');
    } catch (error) {
      console.error('❌ [ConsultationBooking] Navigation error:', error);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="text-center mb-10">
        <h2
          className={`text-3xl font-cinzel font-black uppercase tracking-widest mb-3 ${
        ...
        <p
          className={`font-lora italic text-lg ${
            isLight ? 'text-amber-800/70' : 'text-amber-200/60'
          }`}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {consultationOptions.map((option) => (
          ...
          >
            ...
            {/* Title */}
            <h3
              className={`text-lg font-cinzel font-black uppercase tracking-wider text-center mb-2 leading-tight ${
            ...
            {/* Description */}
            <p
              className={`text-sm text-center mb-4 font-lora min-h-[3rem] ${
            ...
            {/* CTA Button - WITH DEBUG */}
            <button
              onClick={(e) => {
                console.log('👆 [ConsultationBooking] Raw click event:', e);
                handleConsultationClick(option);
              }}
